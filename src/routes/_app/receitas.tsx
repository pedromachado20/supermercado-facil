...
type Ingredient = { id: string; name: string; quantity: string | null; unit: string | null; sortOrder: number | null }
...
const CATALOG = [
  { emoji: '☀️', category: 'Café da Manhã', recipes: ['Tapioca Simples', 'Ovos Mexidos', 'Mingau de Aveia', 'Iogurte com Granola', 'Torrada com Ovo', 'Achocolatado Caseiro'] },
  ...
  { emoji: '🔥', category: 'Airfryer', recipes: ['Pizza na Airfryer', 'Pão de Queijo na Airfryer', 'Batata Frita na Airfryer', 'Frango Crocante na Airfryer', 'Cookies na Airfryer', 'Bolo Simples na Airfryer'] },
  { emoji: '🥤', category: 'Bebidas', recipes: ['Milkshake', 'Chocolate Quente', 'Cappuccino Caseiro', 'Vitamina de Banana', 'Suco Natural'] },
  { emoji: '💪', category: 'Fitness', recipes: ['Panqueca Fitness de Banana', 'Frango Grelhado Fitness', 'Salada Fitness Simples', 'Vitamina Fitness', 'Omelete Fitness', 'Batata Doce Assada', 'Sanduíche Fitness', 'Bolo Fitness de Banana'] },
  { emoji: '⚡', category: 'Receitas Rápidas', recipes: ['Omelete', 'Miojo Melhorado', 'Sanduíche Natural', 'Crepioca', 'Tapioca Recheada'] },
  { emoji: '🍢', category: 'Tira Gostos e Petiscos', recipes: ['Bolinha de Queijo', 'Coxinha', 'Batata Frita Crocante', 'Enroladinho de Salsicha', 'Torrada com Alho', 'Dadinho de Tapioca', 'Carne Louca no Pão', 'Espetinho Simples'] },
  { emoji: '🍔', category: 'Hambúrgueres', recipes: ['Hambúrguer Artesanal', 'Hambúrguer Fitness', 'Hambúrguer Vegetariano'] },
]
...
const PRINT_STYLE = `
  ...
  .desc { font-style: italic; margin-bottom: 1rem; color: #374151; }
  .chips { display: flex; gap: 1rem; margin-bottom: 1rem; font-size: 0.85rem; color: #374151; }
  .section { font-weight: 700; font-size: 1rem; margin: 1rem 0 0.5rem; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.25rem; }
...
async function imprimirReceitas(list: RecipeItem[]) {
  const ings: Record<string, Ingredient[]> = {}
  for (const r of list) {
    const rows = await buscarIngredientes({ data: { recipeId: r.id } })
    ings[r.id] = rows
  }
  const win = window.open('', '_blank')
  if (!win) return
  const data = new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' })
  const corpo = list.map(r => {
    const ingList = ings[r.id] ?? []
    const ingHtml = ingList.length
      ? `<div class="section">Ingredientes</div><ul>${ingList.map(i => `<li>${i.quantity ? `${i.quantity} ${i.unit ?? ''}`.trim() + ' — ' : ''}${i.name}</li>`).join('')}</ul>`
      : ''
    const chips = [
      r.prepTimeMinutes ? `⏱ Preparo: ${fmtTime(r.prepTimeMinutes)}` : '',
      r.cookTimeMinutes ? `🔥 Cozimento: ${fmtTime(r.cookTimeMinutes)}` : '',
      r.servings ? `👥 ${r.servings} porção${r.servings !== 1 ? 'ões' : ''}` : '',
    ].filter(Boolean).join(' &nbsp;·&nbsp; ')
    return `<div class="receita">
      <h1>${r.name}</h1>
      <div class="meta">${r.category ?? 'Supermercado Fácil'} · ${data}</div>
      ${r.description ? `<div class="desc">${r.description}</div>` : ''}
      ${chips ? `<div class="chips">${chips}</div>` : ''}
      ${ingHtml}
      ${r.instructions ? `<div class="section">Modo de Preparo</div><pre>${r.instructions}</pre>` : ''}
    </div>`
  }).join('<hr>')
...
function ReceitasPage() {
  ...
  const { data: receitas = [], isLoading } = useQuery({ queryKey: ['receitas'], queryFn: () => listarReceitas(), staleTime: 0 })
  const { data: ingredientes = [] } = useQuery({
    queryKey: ['ingredientes', receitaSelecionada?.id],
    queryFn: () => buscarIngredientes({ data: { recipeId: receitaSelecionada!.id } }),
    enabled: !!receitaSelecionada,
  })
  ...
  function startEdit(originalName: string) { setEditingRow({ originalName }); setEditingValue(renames[originalName] ?? originalName) }
  function confirmEdit() { if (editingRow && editingValue.trim()) rename(editingRow.originalName, editingValue.trim()); setEditingRow(null) }
  function confirmarAddToCat(catKey: string, isCatalog: boolean) {
    const n = addToCatInput.trim(); if (!n) return
    if (isCatalog) addExtra(catKey, n); else addNameToCat(catKey, n)
  ...
  const totalCadastradas = receitas.length
  ...
  function renderIndexRow(originalName: string, catName = '') {
    ...
    return (
      <div key={originalName} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderBottom: '1px solid var(--color-border)' }}>
        {isEditing ? (
          <>
            <input className="input" style={{ flex: 1, height: '2rem', fontSize: '0.875rem' }} value={editingValue}
          ...
          <>
            ...
            <button className="btn btn-ghost btn-sm" title="Renomear" style={{ padding: '0.2rem 0.4rem', color: 'var(--color-text-soft)' }} onClick={() => startEdit(originalName)}>
              <Pencil size={13} />
            </button>
            {reg ? (
              <>
      ...
      <div style={{ padding: '0.5rem 1rem' }}>
        {addToCatKey === catKey ? (
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <input className="input" style={{ flex: 1 }} placeholder="Nome da receita..." value={addToCatInput}
          ...
          <button className="btn btn-ghost btn-sm" style={{ width: '100%', justifyContent: 'center', color: 'var(--color-text-soft)' }}
    ...
    <div style={{ maxWidth: '860px' }}>
      <div className="page-header">
        <div>
          ...
          <p style={{ color: 'var(--color-text-soft)', fontSize: '0.875rem', marginTop: '0.25rem' }}>
            {totalCadastradas} receita{totalCadastradas !== 1 ? 's' : ''} cadastrada{totalCadastradas !== 1 ? 's' : ''}
          </p>
        </div>
        <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-secondary" onClick={() => setModal('categoria')}><FolderPlus size={15} /> Nova categoria</button>
          <button className="btn btn-primary" onClick={() => abrirNovo()}><Plus size={16} /> Adicionar</button>
        </div>
      </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {receitasAvulsas.map(r => (
                  <ReceitaRow key={r.id} receita={r} marcada={selecionadas.has(r.id)}
          ...
          {CATALOG.map(({ emoji, category, recipes: catRecipes }) => {
            ...
            return (
              <div key={category} style={{ background: '#fff', borderRadius: '0.75rem', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
                <button style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1rem', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}
                  ...
                  {regCount === total && total > 0
                    ? <span className="badge badge-green">{regCount} ✓</span>
                    : regCount > 0 ? <span className="badge badge-green">{regCount}</span> : null}
              ...
              <div key={cat.id} style={{ background: '#fff', borderRadius: '0.75rem', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
                <button style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1rem', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: '0.8rem', color: 'var(--color-text-soft)' }}>
                      {total === 0 ? 'Nenhuma receita ainda' : regCount > 0 ? `${regCount}/${total} cadastradas` : `${total} receita${total !== 1 ? 's' : ''}`}
                    </p>
                  </div>
                  {regCount > 0 && <span className="badge badge-green">{regCount}</span>}
                  <button className="btn btn-ghost btn-sm" style={{ color: 'var(--color-danger)', padding: '0.25rem' }}
      ...
      {modal === 'ver' && receitaSelecionada && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal modal-lg" onClick={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
            <div className="modal-header" style={{ flexShrink: 0 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <h3 style={{ fontWeight: 700, fontSize: '1.125rem' }}>{receitaSelecionada.name}</h3>
                {receitaSelecionada.category && (
                  <span style={{ fontSize: '0.75rem', color: 'var(--color-text-soft)' }}>{receitaSelecionada.category}</span>
                )}
              </div>
            ...
            <div className="modal-body" style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>

              {/* Descrição */}
              {receitaSelecionada.description && (
                <p style={{ color: 'var(--color-text-soft)', fontStyle: 'italic', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>
                  {receitaSelecionada.description}
                </p>
              )}

              {/* Chips de tempo/porções */}
              {(receitaSelecionada.prepTimeMinutes || receitaSelecionada.cookTimeMinutes || receitaSelecionada.servings) && (
                <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                  {receitaSelecionada.prepTimeMinutes && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '2rem', padding: '0.35rem 0.75rem', fontSize: '0.8125rem', fontWeight: 600, color: '#15803d' }}>
                      <Clock size={13} /> Preparo: {fmtTime(receitaSelecionada.prepTimeMinutes)}
                    </div>
                  )}
                  {receitaSelecionada.cookTimeMinutes && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '2rem', padding: '0.35rem 0.75rem', fontSize: '0.8125rem', fontWeight: 600, color: '#c2410c' }}>
                      <UtensilsCrossed size={13} /> Cozimento: {fmtTime(receitaSelecionada.cookTimeMinutes)}
                    </div>
                  )}
                  {receitaSelecionada.servings && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '2rem', padding: '0.35rem 0.75rem', fontSize: '0.8125rem', fontWeight: 600, color: '#1d4ed8' }}>
                      <Users size={13} /> {receitaSelecionada.servings} porção{receitaSelecionada.servings !== 1 ? 'ões' : ''}
                    </div>
                  )}
                </div>
              )}
              ...
              {ingredientes.length > 0 && (
                <div>
                  <h4 style={{ fontWeight: 700, fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--color-text-soft)', marginBottom: '0.625rem' }}>
                    Ingredientes
                  </h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    {ingredientes.map(ing => (
                      <div key={ing.id} style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', padding: '0.375rem 0', borderBottom: '1px solid var(--color-border)' }}>
                        {(ing.quantity || ing.unit) && (
                          <span style={{ fontSize: '0.8125rem', fontWeight: 700, color: 'var(--color-primary)', minWidth: '5rem', flexShrink: 0 }}>
              ...
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div className="form-group">
                  ...
                  <select className="input" value={categoria} onChange={e => setCategoria(e.target.value)}>
                    ...
                    {CATALOG.map(c => <option key={c.category} value={c.category}>{c.emoji} {c.category}</option>)}
                    {customCats.map(c => <option key={c.id} value={c.category}>{c.emoji} {c.category}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label className="label">Receita (cole o texto aqui)</label>
                  <textarea className="input" placeholder="Cole aqui o texto completo da sua receita..." value={texto} onChange={e => setTexto(e.target.value)} rows={14}
...
function ReceitaRow({ receita, marcada, onToggle, onVer, onImprimir, onExcluir }: {
  ...
  return (
    <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', padding: '0.875rem 1.25rem', border: marcada ? '2px solid var(--color-primary)' : '1px solid var(--color-border)', background: marcada ? 'var(--color-primary-bg)' : '#fff' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={onVer}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <ChefHat size={16} color="var(--color-primary)" />
          <span style={{ fontWeight: 600, fontSize: '0.9375rem' }}>{receita.name}</span>
        </div>
        {receita.description && (
          <p style={{ fontSize: '0.8rem', color: 'var(--color-text-soft)', marginTop: '0.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {receita.description}
          </p>
        )}
      </div>
